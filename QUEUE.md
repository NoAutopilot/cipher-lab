...
## Re-rank for LANE B5, 26 Sept 2026, 00:03 UTC

Worker SCOUT-RERANK (Sonnet), for parent 7d (session_01744aLgcLnadR1XQckyHwcu). LANE B4 closed 23:57 UTC 25 Sept 2026
with every UNSOLVED-SURVEY.md rank 1-30 first-tested and ranks 31-41 giving no spec (bSPEC3 triage,
`specs/cheap-tests/specs31-41-triage.tsv`), so the survey list itself has nothing left to hand a breadth worker.
This re-rank does not repeat that survey; it reads the rest of QUEUE.md (pools, GitHub-held candidates, the LANE
KX/VX/N4 sections) for cryptanalysis- or recovery-kind candidates with no spec yet, or a spec whose
`cheap_tests_in_order` still names an unrun step, per CLAUDE.md's selection rule (expected value = P(first cheap
test moves it) x value / cost; pools first). No web fetch was needed this pass; every row below is read from files
already in the repository (QUEUE.md itself, `specs/*.json`, NEAR.md, STATUS.md's LANE B4/ZX2 handoffs,
`tools/data/`'s corpus list).

**Excluded from the ranking, not re-offered:** `antt-linhares-chave`, `antt-msliv0638-brochado-1712`,
`catokwacopa-1875`, `destaing-gerard-1779`, `fr3985-nevers-f176`, `na-janssens-java-1811`,
`na-schonenberg-1678-1716`, `na-suriname-map-1781` all show `cheap_test_done` unset in their spec JSON, which
made them look like unrun first tests by that field alone — but each one's own `cheap_tests_in_order` and NEAR.md/
STATUS.md entries show a lane (PX, LX, VX, ZX2, R5) already ran and logged that very test this week (period-key
decodes graded C/M/U, or a control-backed negative on file); the JSON field was simply never written back by
those lanes' own tooling. Re-running them here would be the duplicate work CLAUDE.md's room rule exists to
prevent — they stay with their owning lanes.

| Rank | Target | Source row | Kind | Signs/N | Lang. + corpus | Named first cheap test + matched control | Est. cost | Pool? | Why it ranks here |
|---|---|---|---|---|---|---|---|---|---|
| 1 | Forget/Mayenne/Matignon despatches (BnF fr.15572 + fr.15571 f.177, 13 folios) | QUEUE.md G1 (github-held, `dbourdeau/cyphersolver` `matignon1586`) | recovery | 12,994 profile tokens, 13 folios; largest transcribed block on file | fr, fr16 (16th c., era-matched) | Apply the already-verified Mayenne-Forget Cipher-1 key (beats a scrambled-key control by +2.07/figure on ff.123-124) to the leaves transcribed only in part (f.110, ff.123-124, ff.143/150/154/173/196/201, f.177); control: the same key on a scrambled-cipher-order copy of the same leaves. Separately: transcribe the already-photographed interlinear decipherment of Cipher-3 (`img/BnFfr15571f179.jpg`, magenta ink) directly, no key needed. | $4, Sonnet | **yes** — one office (Mayenne/Forget), one key family, >12,000 signs | A working, already-verified key with unread leaves behind it is the strongest EV on the list: near-certain P(moves), and the Cipher-3 leaf is a transcription job, not cryptanalysis at all |
| 2 | Lope Hurtado de Mendoza (Rome) to Charles V, 3 untested sibling records (RAH Salazar 9/26, DECODE R9634/R9646/R9649) | QUEUE.md G2 (github-held, `lopehurtado`) | recovery | 6,289 profile tokens across the 9-record office; 3 records untried against the key | es; no 16th-c. Spanish corpus on disk (`es17`/`es17c` are 17th-c. — era mismatch, flag before trusting any judge score, cf. rule 3's pt17/pt18 lesson) | Apply the 49 code values recovered from R9644's own contemporary clear copy (`key_codes.tsv`) to R9634, R9646 and R9649; control: the same key applied to 3 random-digit strings of matching length/alphabet. Judge by eye (grade C/M per token) rather than the `es17c` language judge given the era mismatch. | $3, Sonnet | **yes** — one office, one key, key already partially confirmed | A real period key exists and is simply unapplied to 3 of 9 records in the same box; Aymeloglu's `bne-ranked.md` also names 2 further unread Lope Hurtado letters (MSS/18697/29, MSS/20212/27) as a possible pool extension |
| 3 | Principe di Castelcicala despatches to Naples, un-glossed 1816 letters (ASNa Esteri 2337, DECODE R9553-R9589) | QUEUE.md G3 (github-held, `castelcicala1816`) | recovery | 10,397 profile tokens; ~2,450-group two-part numeric code; 162 values already confirmed from 4 glossed records | it; `it16` on disk is 16th-c. — a 300-year era mismatch for an 1816-23 target, worse than the pt17/es17c cases already logged; do not judge with it16 without flagging this, and prefer a hand read against the Decazes/Florida crib themes over any language-model score | Hand-read the un-glossed 1816 letters against the 162 already-confirmed code values and the Decazes/Florida crib vocabulary (the escalation log's own "retry" step, marked not-done); no control needed for a crib-driven hand read, only for any later blind anneal | $4, Sonnet or strongest model for the read | **yes** — one office, one two-part code, >10,000 signs | Confirmed values already exist for close to half the record set; the one un-run step is named in the repository, not invented here |
| 4 | Riksarkivet Stockholm cipher group, R4282 vs R4284's own key-test leaf (DECODE R4282-R4341) | QUEUE.md G4 (github-held, `riksarkivet1628`) | recovery | 2,622 profile tokens across 4 open documents | de/la; de16 era-fits 1628-33 reasonably, la_repo generic Latin | Use R4284's own key-test leaf (four Latin phrases written over their cipher equivalents) as a crib against R4282, which may share R4284's key; control: the same crib alignment attempted against 3 of the other fetched Chifferklaver key records (68 fetched, only 2 ever tried against any of the three open documents) | $4, Sonnet | not confirmed — "four unrelated correspondences bundled as one catalogue row" per G4's own note; R4282/R4284 key-sharing is a hypothesis, not established | A concrete, named, never-tried crib source (R4284's own key-test leaf) sitting beside an untested document in the same bundle |
| 5 | Biblioteca RAH, Signatura 9/23 (+9/24-9/25) Spanish cluster | QUEUE.md scPOOL "real pools" table | recovery, pre-spec | 133 + 38 = 171 letters, ~749+ pages; signs unmeasured | es; same era-corpus gap as row 2 (no 16th-c. Spanish corpus) | Not a cryptanalytic test yet: sample 3-5 pages via DECODE or RAH's own viewer, count real signs per page and confirm copy-free status, before any spec is written | $3, Sonnet | **candidate pool, unmeasured** — scPOOL's own top follow-on lead (24 Sept 2026), same era as CATALOG.md's Adrian 1521/Juan Manuel 1521-22 items | Highest-EV *pre-spec* lead on file: a genuine multi-letter cluster (171 letters) nobody has sampled for a real sign count, which is the one fact standing between it and a spec |
| 6 | BAV, Barb.lat 6956 | QUEUE.md scPOOL "real pools" table | recovery, pre-spec | 98 letters, 1628-1629, ~542 pages; signs unmeasured, "unexamined by us" | not yet determined (Vatican correspondence, likely it/es/la) | Sample 3-5 pages for language, sign count and copy-free status before any spec | $3, Sonnet | **candidate pool, unmeasured** | Second-largest real pool on file after row 5, genuinely untouched |
| 7 | ARA Brussels, Secretairerie/Carpio-Fuenmayor | QUEUE.md scPOOL "real pools" table | recovery, pre-spec | 44 letters, 1674-1678, ~186 pages; signs unmeasured | not yet determined | Sample 3-5 pages for language, sign count and copy-free status; named as "one correspondent pair", the cleanest single-key-family shape of the unmeasured pools | $3, Sonnet | **candidate pool, unmeasured, strongest shape** — one correspondent pair, one likely key | A named single correspondent pair is the shape most likely to actually be one key family once measured, ahead of the two ASV rows below which are archive-series groupings, not one relationship |
| 8 | ASV i1025 SdS France 17-18 | QUEUE.md scPOOL "real pools" table | recovery, pre-spec | 44 letters, 1580-1589, ~193 pages; signs unmeasured | fr, fr16 would era-fit if the letters turn out 16th-c. | Sample 3-5 pages for language, sign count and copy-free status | $3, Sonnet | **candidate pool, unmeasured** | Vatican Secret Archive series-level pool, untouched |
| 9 | ASV i1025 SdS Spain 364C | QUEUE.md scPOOL "real pools" table | recovery, pre-spec | 45 letters, 1717-1720, ~186 pages; signs unmeasured | es; es17c (17th c.) is close but still a decade+ short of 1717-20 — check before trusting a judge score | Sample 3-5 pages for language, sign count and copy-free status | $3, Sonnet | **candidate pool, unmeasured** | Same family as row 8, later date range |
| 10 | Benedetto Fantini ("Jantini") to the Este court (ASMo Ambasciatori Ungheria b.4, DECODE R1126-R1127) | QUEUE.md G12 (github-held, `jantini1517`) | cryptanalysis | 581 profile tokens over 2 records | it; it16 (16th c.) era-fits 1517-18 well, unlike row 3 | Bourdeau's own control-validated ciphertext-only syllabary anneal already fails here with a passing control on synthetic text (closed, for the record) — listed only so a future worker does not re-run the same annealer; no untried lever named in the repository | n/a — negative already on file | no | Included for completeness of the "no spec" sweep, not as a live candidate: QUEUE.md's own honest assessment already calls this the negative case in its batch |

**Not now** (famous items, no named untried cheap test, stay with LANE GOLD or out of scope per the 25 Sept 2026
B4/bSPEC3 triage): Shugborough inscription (no mechanical test for a 10-letter initialism), Zodiac Z13/Z32 (the
one on-record mechanism already control-tested closed-negative, 24 Sept 2026), Kryptos K4 (plaintext already
found privately, cryptanalysis moot), Beale cryptograms, Rohonc Codex (needs Hungarian and years, not a cheap
test), Codex Seraphinianus (not a cipher by the author's own statement), Voynich manuscript, WW2 pigeon message
(GCHQ's own one-time-pad assessment), Double Column Transposition "reloaded" and the 65-bit world-record
challenge (artificial compute challenges, out of project scope), Bonus 22 / M-209 series (campaign-scale, no
m209 tool on disk).

Rows 5-9 are pre-spec: they need a $3 sampling pass each before `specs/<slug>.json` can be written at all, which
is why they sit below rows 1-4 (already-specced, already-keyed) despite two of them (rows 5, 7) being the
strongest pool shapes on file. Kind: scout (this section files leads and re-ranks only; it does not promote to
the board, run any test, or classify novelty, per rule 10 and this worker's brief).
...
## Scored backlog for LANE B6, second pass, 26 September 2026 03:15 UTC

Worker bSCOUT6 (Sonnet), for LANE B6 orchestrator (session_0125z25Ujw4ehAavXcyAfwXz). Fresh shallow clones:
`dbourdeau/cyphersolver` HEAD `fc0c9e865d0fae67ca92d19750d2b09ab11972e0` (25 Sept 2026 18:14 local/-0500), and
`aaymeloglu/unsolved-ciphers` HEAD `2495c45e8b94ffbc4f09a085224aa5ebce5cdf9f` (23 Sept 2026 14:27 local/-0500).
`tools/solver_repo_diff.py` run in queue mode against these clones and the existing QUEUE.md rows: no row changed
status beyond what the 23 Sept 2026 diff (commit `2e9ec01`) and LANE B5's 00:03 UTC re-rank already recorded — the
"already solved" hits it returns are all on multi-record BL Add MS 4136/32091/Harley MS 1582 rows already annotated
with those same sub-target names (moray, coligny1563, stafford1586, etc.) from the 23 Sept pass; nothing new to drop.

**The actual find this pass** is not a status change on our existing rows but four items harvested from
Bourdeau's own on-disk `CATALOGUE.md` (his "New-solves sweep of 22 Sept" and "Archive scan of 23 Sept" sections,
57 open targets he lists but has not read) that are absent from our `ciphers/`, `specs/`, `QUEUE.md` and `NEAR.md`
(checked by name/shelfmark grep across all four, and against ROOM.md's last 200 lines and LANE AX/GOLD3/GOLD4's
held targets — no collision). Per CLAUDE.md item 8 (Access playbook), Bourdeau's repository is MIT code / CC BY 4.0
text: cited here with folder and date read, no code copied. Aymeloglu's own `CATALOGUE.md` (PARES/BNE-heavy) was
also read this pass but adds nothing new reachable from the cloud: PARES is a dead host (Access playbook table) and
its BNE leads need the same login-gated images already logged elsewhere.

| Rank | Target | Source | Kind | Signs/N | Lang. + corpus | Bourdeau/Aymeloglu fraction read | First cheap test + matched control | Est. cost | Pool? |
|---|---|---|---|---|---|---|---|---|---|
| 1 | Otto von der Malsburg (Hesse-Kassel General Commissary) to the Hesse-Kassel government, 10 cipher letters on the Westphalian war, Dec 1636 – 1637 | Hessisches Staatsarchiv Marburg, HStAM 4 h Nr. 1411, ff.3-33; images via HCPortal's own image API (`api.hcportal.eu/media/...`), confirmed reachable by Bourdeau's own session, e.g. f.3 (`.../media/1392/35901677515129.jpg`); HCPortal records 496-509, all marked "Not solved" (Bourdeau CATALOGUE.md #338, read 26 Sept 2026) | cryptanalysis | ~250 two-digit groups on f.3 alone, "perhaps 2,000+ groups" across all ten letters (Bourdeau's own estimate) | de; de16/de20 both era-mismatched by a century+ either side of 1636-37 — flag before trusting any judge score, grade by eye first | 0 — Bourdeau viewed f.3 only (no cryptanalysis attempt logged); not in Aymeloglu's catalogue at all | Fetch all 10 letters via the HCPortal image API (new host, not yet in the Access playbook table — test reachability with a single `curl -sS -o /dev/null -w "%{http_code}"` first, log the result there), transcribe, then a flat IC/frequency test against a synthetic German control of the same length and design (masc or homophonic, whichever the two-digit-group + capital-code-sign structure fits) | $4, Sonnet (transcription) + $2 (IC/control) | **yes** — one office (Malsburg to the Hesse-Kassel councillors), one war correspondence, single system, ~2,000+ groups is the largest never-attempted single-key pool this pass found |
| 2 | Siena, Concistoro "Lettere in cifra": the still-open pieces (nos. 6/24, 20/23 — two systems, ~9,000 tokens — plus nos. 7, 9, 11, 15, 17, 19, 21) | Archivio di Stato di Siena, Concistoro 2308 fasc. 2; on DECODE (keys R4746-R4789, letters R4790-R4814, all with images); Bourdeau CATALOGUE.md #336, read 26 Sept 2026 | recovery (siblings in the same busta already partly keyed by Bourdeau: no. 25 ciphertext-only-recovered and 96% read, no. 14 partial, no. 4 partial) | ~9,000 tokens across the two open systems (nos. 6/24, 20/23) plus 5 shorter open pieces | it; `it16` on disk is 16th c. — these letters are 15th/early-16th c. (busta spans back to 1421), a real era gap wider than the pt17/es17c precedent (grade by eye, flag the mismatch if a judge score is pasted at all) | 0 on the specific open pieces — Bourdeau's own recovered keys (nos. 25, 14, 4) have not been tried against nos. 6/24, 20/23, 7, 9, 11, 15, 17, 19, 21; not in Aymeloglu's catalogue | Try Bourdeau's no. 25/no. 14/no. 4 recovered key values against the shorter open pieces (7, 9, 11, 15, 17, 19, 21) first — cheapest, needs no new cryptanalysis, matched control is the same key applied to a scrambled-cipher-order copy of the same piece; only if that fails, ciphertext-only anneal on nos. 6/24 and 20/23 with a synthetic-Italian control of matching length/design | $3, Sonnet (key-transfer try) then $5 if the anneal is needed | **yes** — one archive series, 25 photographed pieces, 42 keys in the same busta (per Bourdeau's own count) |
| 3 | Mniszech–Sieniawski–Schenck letters, three groups: Moldavian/Wallachian news to Hetman Sieniawski, two unidentified, and 15 Dresden letters to Schenck | Archiwum Narodowe Kraków (DECODE R7468-R7500, 33 records, all DECODE status N/A, no attached decryption); Bourdeau CATALOGUE.md #333, read 26 Sept 2026 (distinct from the already-tracked Potocka/Mniszech-to-Dunin cluster, R7524-7536, which is NOT in this range) | recovery/cryptanalysis, pre-spec | 33 records unmeasured; the 15-letter Dresden-to-Schenck subset is itself a real pool if one hand wrote all 15 | unknown — Polish, German or French all plausible from context (Bender/Poltava 1706-09); no Polish corpus on disk regardless | 0 — DECODE N/A, no reading found by Bourdeau; not in Aymeloglu's catalogue | Sample 3-5 of the 15 Dresden-to-Schenck records via DECODE for language, sign count and copy-free status before any spec | $3, Sonnet | **candidate pool, unmeasured** — the cleanest single-correspondent shape (15 letters, one recipient) of this pass's finds |
| 4 | Heinrich Anton von Zeschau (Dresden) to Albin Leo von Seebach, 4 ciphertexts, 1841-43 | Saxon Main State Archive Dresden (DECODE R5005-R5008); Bourdeau CATALOGUE.md #53, read 26 Sept 2026 | recovery | R5005 transcribed (3,969 digits); R5006-R5008 unmeasured | de; de20 is the nearer-era corpus (check its actual date range before trusting a judge score — not verified this pass) | Bourdeau recovered 7 partial syllabary values (la, pre, m, i, er, e, que) from R5005's own pencil rubbing but did not try them against R5006-R5008 (same correspondent pair); ciphertext-only solvers failed on R5005 alone, no matched control stated; not in Aymeloglu's catalogue | Apply the 7 recovered values to R5006-R5008 as a crib (no anneal needed for this step); control: the same 7-value substitution applied to a random-digit string of matching length | $3, Sonnet | **yes** — one correspondent pair, 4 ciphertexts, partial key already in hand |
| 5 | Szembek family papers: a Latin cipher letter, early 18th c. | Biblioteka Kórnicka BK 1560 (Wielkopolska Digital Library, free images); Bourdeau CATALOGUE.md #348, read 26 Sept 2026 | recovery/cryptanalysis, pre-spec | unmeasured — Bourdeau logged "length and system not yet checked" | la; `la` generic corpus on disk | 0 — no reading found, images not yet opened by anyone; not in Aymeloglu's catalogue | Fetch the image from the Wielkopolska Digital Library, count signs, confirm language and copy-free status before any spec | $2, Sonnet | no — single letter |
| 6 | Hessian polyalphabetic message, 20 Feb 1824 | Hessisches Staatsarchiv Marburg, HStAM 9 a Nr. 259 f. 249 (HCPortal record 513, one image online, marked "not solved" as polyalphabetic); Bourdeau CATALOGUE.md #340, read 26 Sept 2026 | cryptanalysis, pre-spec | one page, unmeasured | de; era close to de20 if that corpus is in fact 19th/early-20th c. (verify before use) | 0 — not attempted; not in Aymeloglu's catalogue | Fetch the single HCPortal image, transcribe, confirm the polyalphabetic design (rare enough on its own to be worth a look) before any anneal | $2, Sonnet | no — single page, but a design (polyalphabetic) uncommon enough in this project's holdings to be worth the cheap look |
| 7 | Hesse-Kassel/Denmark letter with enciphered passages, 4 May 1672, partly solved | Hessisches Staatsarchiv Marburg, HStAM 4 f Dänemark Nr. 125, ff.2-4 (HCPortal record 494, 3 images online, marked "Partially solved" with no stated reader or method); Bourdeau CATALOGUE.md #341, read 26 Sept 2026 | contribution (completion of an already-partial reading) | unmeasured, short (3 leaves) | de/da; da19 on disk if the correspondence turns out Danish-side | 0 — HCPortal's own "partial" is unattributed; Bourdeau did not chase who read what | Fetch the 3 images, read what portion is already solved (HCPortal record page or an attached note) before deciding whether the rest is a cheap completion or blocked without the original solver's key | $2, Sonnet | no |
| 8 | Nuncio Bagno (Paris) to the Secretariat of State, 5 Jan 1652 | AAV Segr. Stato Francia 104 (DECODE R5622, 5 images: 1 clear leaf + 4 pages of figures); Bourdeau CATALOGUE.md #298, read 26 Sept 2026 | cryptanalysis | 4 cipher pages, unseparated decimal figures, sign count unmeasured (Bourdeau's OCR was too noisy for a transcript) | it/la mixed (clear Italian phrases inside the cipher pages); Lasry's Francia 346 key structurally excluded (separator digit collides) | 0 — attempted by Bourdeau, no reading, no key found, no matched control stated; not in Aymeloglu's catalogue | Transcribe the 4 cipher pages by hand from the DECODE images (OCR failed; a model reading the image directly may do better than OCR), then the standard nomenclator/homophonic sweep with a matched synthetic-Italian control | $4, Sonnet (transcription) + $4 if a sweep is warranted | no |

**Not offered** (no route to an image at all, so no cheap test exists yet — an ASKS.md row for the person, not a
queue row): Florence 1414 letters (Bourdeau CATALOGUE.md #335, key transcribed but the letters themselves are not
digitised, not on DECODE, not on the Yale Ilardi reels), Siena Concistoro no. 1 of 1421 (#336, unphotographed),
Medici/Guicciardini 1425 (#337, ASF image server unreachable, not digitised), Ungnad 1576 and Sinzendorff 1578
imperial despatches from Constantinople (#342, #343, ÖStA has no scans, no ID online), Esterházy family archive
box 636 (#344, "scans may not be online yet"). None is entered above per the breadth-lane rule (stop at the first
block, do not spec a target with no reachable image).

Scored per `.claude/briefs/scout.md`'s reader profile (any of English/French/Italian/Spanish/Latin/German/Dutch/
Portuguese scores language_fit 3) and CLAUDE.md's pools-first selection rule (a 2,000+-sign single-key pool outranks
a single short letter): row 1 (Malsburg, ~2,000+ signs, one office) ranks above row 2 (Siena, ~9,000 tokens but two
systems and needing a key-transfer step first) only because row 1's first test needs no prior key at all, while row
2's cheapest path leans on someone else's (Bourdeau's) recovered values, which is recovery, not our own cryptanalysis,
until it fails. Hosts touched this pass: github.com (2 shallow clones, deleted after use), no other host fetched
(HCPortal, DECODE and the Wielkopolska Digital Library are named as the next worker's first test, not fetched here).
Never promoted to the board; no target attempted; no novelty classified.
...
> G10 update, LANE NX 26 Sept 2026: fr.3151 no.33 (Noailles, Venice 1558, to the Cardinal de Lorraine) now has its own folder `ciphers/fr3151-noailles-1558` (check-solved open, intake gate exit 0): three cipher blocks on f.60r/f.60v, about 200-230 signs, each with a period marginal gloss; gloss-crib key job NX-3151G live.
...
| D8 | Luis Fernández (Rome) letters, 1523, RAH Signatura 9/29 | 1523 | es | recovery | R9838, R9839, R9840, R9842, R9843 / R9841, R9834 (Decrypted) | Only survivors of the volume-aware exclusion; same hand as Bourdeau's 9/28 and 9/30 targets, so expect him to reach 9/29 | **LANE DX check-solved, 25 Sept 2026 (NB-ES): found-solved, Bourdeau's sessa1524; R9834 mined as crib. ciphers/rah9-29-fernandez-1523/NOTES.md.** Read only if DECODE opens before he gets there; otherwise a duplicate | 30 |
...
| D2 | Rome embassy correspondence (Lope Hurtado de Mendoza's circle; several letters signed "Luis Fernández") to Charles V/court, RAH Signatura 9/31 | 1524 (13 of the shelfmark's items undated in the catalogue) | es | recovery | R9872, R9875, R9879, R9880, R9882, R9883, R9885-R9889, R9891, R9892, R9894 / R9872's, R9881's, R9884's, R9890's, R9893's and R9895's own Decrypted status (6 different neighbours, one per sub-cluster) | Same shelfmark (BRAH Signatura 9/31), each Non-decrypted item a folio away from a Decrypted sibling; same correspondent name recurs across the legajo | **LANE DX check-solved, 25 Sept 2026 (NB-ES): found-solved, inside Bourdeau's live sessa1524 campaign (RAH 9/28-9/34 working key); D6 folded here. ciphers/rah9-31-fernandez-1524/NOTES.md.** Same correspondent and RAH Signatura 9.x series as QUEUE row G2 (`ciphers` none yet, `cs-recheck/lopehurtado/`, Signatura 9/26) — check whether G2's recovered `key_codes.tsv` alphabet (49 confirmed values) reads any of these before assuming a new key; if not, log in to DECODE and read the six Decrypted siblings first | 32 |
| D3 | Same "Luis Fernández" Rome-embassy hand, RAH Signatura 9/34 | 1525 | es | recovery | R9896, R9899, R9901 / R9898 | Same shelfmark, same year, one folio-adjacent Decrypted neighbour (R9898) for all three | **LANE DX check-solved, 25 Sept 2026 (NB-ES): found-solved, Bourdeau's sessa1524; R9898 already read. ciphers/rah9-34-fernandez-1525/NOTES.md.** Same as D2: try G2's key first, then read R9898's decipherment as a crib | 32 |
| D4 | Bernardino Naro / the "Nuncio Damiata" at the Paris nunciature, to the Secretariat of State, doss. 62, next to a Partially decrypted item in doss. 64 of the same fond | 1624-1625 | it | recovery | R59, R60, R61 / R71 | Same fond (Vatican Secret Archive i.1025, Segretario di Stato, France), dossier numbers 3 apart, dates 6-7 weeks apart, same recipient ("the Secretariat") | **LANE DX check-solved, 25 Sept 2026 (NB-FI): found-solved, Bourdeau damiata1624/ (21 Sept 2026) reads R59-R61 from the leaf's interlinear decipherment. ciphers/asv-naro-paris-1624/NOTES.md.** Register on DECODE, read R71's partial decipherment, apply to R59-R61; check Bernardino Spada's and Naro's own printed correspondence (Bibl. Vat. and Treccani entry cite a documented 1625 Paris nunciature) for a contemporary abstract first | 32 |
| D5 | Ferdinand (the future Ferdinand II of Aragon) to his father John II of Aragon | 1478 | es | recovery | R1172 / R1180 | Same shelfmark (BNE MSS/20211), items 3 apart, both catalogued 1478 | **LANE DX check-solved, 25 Sept 2026 (NB-ES): blocked -- Tomokiyo 2018 and Galende Díaz 1993-94 located, not read; unclaimed by either repository; R1172/R1180 being fetched by the DX login worker. ciphers/bne20211-ferdinand-1478/NOTES.md.** **Check print first**: a paper titled "Spanish Ciphers before Accession of King Ferdinand: 1470-1479" (found via web search, not yet read) appears to already discuss and reconstruct Ferdinand's 1470s/1476-79 ciphers to John II from this same manuscript family — read it before assuming this is unworked; if it stops short of R1172 specifically, log in to DECODE and read R1180 as the crib | 32 |
| D6 | Extension of QUEUE row G2 (Lope Hurtado de Mendoza), one more item just past the R9634-R9656 range already flagged there | 1522 | es | recovery | R9659 / R9652 | Same shelfmark (BRAH Signatura 9/26) as G2, adjacent folio to the Partially decrypted R9652 | **LANE DX, 25 Sept 2026: folded into D2 (Bourdeau's sessa1524/lopehurtado).** Fold into G2's existing work rather than opening separately — `cs-recheck/lopehurtado/` already has a partial key for this legajo | 28 |
| D7 | Jean de Langeac, bishop of Avranches (French ambassador to Venice from 1528, envoy earlier too), to Anne de Montmorency | c. 1520-1540, this item's own date not separately catalogued | fr | recovery | R3694 / R3695 | Same shelfmark (BnF fr. 3083), same 10-day-or-less date window as the Decrypted R3695, both catalogued to the Montmorency correspondence | **LANE DX check-solved, 25 Sept 2026 (NB-FI): found-solved, Bourdeau gramont1529/fr3083_no8_langeac.md (18 Sept 2026) reads R3694 via Lasry's 2023 Avranches key. ciphers/fr3083-langeac-montmorency/NOTES.md.** Register on DECODE, read R3695's decipherment for the cipher's scheme/key, apply to R3694; BnF's own finding aid (archivesetmanuscrits.bnf.fr) lists fr. 3083 among Montmorency's incoming diplomatic correspondence, worth checking for a contemporary calendar entry first | 32 |
...
|---|---|---|---|---|---|---|---|
| KX-01 | Loménie de Brienne, Secrétaire d'État aux Affaires Étrangères, outgoing despatches, juillet-décembre 1661 — BnF Mélanges de Colbert 26, part III | `ciphers/fr5160-letellier-1653/key_1659.tsv` / `key_brienne_1647.tsv` / `key_brienne_1651.tsv` (Brienne's own cipher, reused fr5160/clair1067) | same office (Brienne) and correspondence chain as fr.5160 (1653-1661), tail end of that volume's own date range; found by LANE G2's sibling-volume search, named there as the closest lead but never opened as an image | yes — Gallica ark `btv1b10035069t` (corrected by KX-COLB26; `cc955062` is the finding-aid id, `btv1b955062` never resolved) | yes | **closed, negative eye-check 25 Sept 2026 (KX-COLB26): 104/375 canvases of part III sampled, no cipher** — see fr5160 NOTES.md 'KX-COLB26' | R5 owns fr5761/Clairambault-adjacent French Secretary-of-State targets — flag for R5 or whichever lane next opens a French-chancery target |
...
| KX-03 | Gaspard Coignet de la Thuillerie (French ambassador, Copenhagen/Stockholm) and Abel Servien (Münster), 1644-1648 — BnF Mélanges de Colbert 26 part I (canvas ~4-69 of Gallica `btv1b10035069t`); cipher passage seen on canvas 20 (letter from Copenhagen, numeral nomenclator inline with clear French) | to test, not assumed: `ciphers/clair1067-brienne-poland-1646/key_1646.tsv`, `key_brienne_1647.tsv` (Brienne's Secretary-of-State office, 1646-47, same years); `fr5160-letellier-1653/key_brienne_1647.tsv` | same office years (Brienne secretary of state 1643-63; La Thuillerie and Servien were his correspondents) and the same volume as KX-01; found by KX-COLB26 on 25 Sept 2026 while resolving the ark | yes | yes | **open** (KX-COLB26P1, 25 Sept 2026: Négociations secrètes 1725-26 vols 1-4 and APW online full text read, neither prints them); 11 enciphered leaves; a second hand's interlinear notes sit near the cipher runs on all 21 cipher-bearing canvases, but at native resolution they are topical paraphrases (e.g. 'Je parle des Suedois' over an 11-token run), not a word-per-code decipherment (KX-LATHKEY2, 25 Sept 2026) -- cribs, not a key; clair1067 key_1646, key_brienne_1647 and fr5160 key_1659_ext do not read canvas 20 (orchestrator look, 25 Sept 10:00, no matched control yet); folder `ciphers/colbert26-lathuillerie-1644/` | LANE KX |
...
Worker VX-SCNORD (Sonnet), Nordic round 2: Riksarkivet Sweden, Rigsarkivet Denmark, KB Denmark, Arkivverket/
Digitalarkivet Norway, National Library of Finland, Alvin (Uppsala/Swedish libraries). Row prefix VX-S. Follows
VX-SCEU (round 1, reachability-only on these hosts). Brief: same target pattern as VX-G/VX-B/VX-E above,
copy-free only.

**Two numbered rows, both copy-free.** Both new working search routes (neither host had a working query route
on file before this pass) are documented in the method notes below for reuse.

| Row | Holding / shelfmark | Date(s) | Sender -> recipient | Undeciphered material (how many letters/lines) | Key or decipherment beside it (what, and exactly where) | Image URL tested (full size) + leaf | Copy-free (yes/no) | Scout note (why EV is high/low; transcription on disk? printed edition to check?) |
|---|---|---|---|---|---|---|---|---|
| VX-S01 | Rigsarkivet (Danish National Archives), Tyske Kancelli Udenrigske Afdeling ("German Chancellery, Foreign Department"), archive series "Chiffer" (heid 2132747) | 1670-1709 (key collection); the flagged undeciphered letters are 1712 | N/A here (the key itself); flagged undeciphered material is chevalier Lagau (French legation secretary, Stockholm) -> comte de La Marck (French ambassador, Stockholm) | N/A for this row (key). **Flag**: the *same archive creator* (Tyske Kancelli, Udenrigske Afdeling) holds a separate, undigitised series, "Opsnappede chifferbreve fra chevalier Lagau" (heid 3245448, "Intercepted cipher letters from chevalier Lagau"), 12 Apr-27 May 1712, one uninventoried "pakke" (box), reference "Fremmed proveniens, Frankrig -- Danske Kongers Historie, fasc. 358". Catalogue note (verbatim, Danish): "Ifølge samtidig, dansk påskrift var chevalier Lagau fransk legationssekretær i Stockholm, og brevene var stilet til den franske ambassadør i Stockholm grev de La Marck." (Per a contemporary Danish endorsement, chevalier Lagau was French legation secretary in Stockholm, and the letters were addressed to the French ambassador in Stockholm, Count de La Marck.) | **(i) key/nomenclator table**, described at series level as "Indeholder en blanding af mange forskellige ciffer-nøgler" (contains a mixture of many different cipher keys); reference "Vejledende Arkivregistraturer XI, s. 49, TKUA Alm. del 3 nr. 112" | `https://arkivalieronline.rigsarkivet.dk/da/billedviser?epid=19852193` -- 309-page scanned volume, cover leaf (p.1/309) reads "Tyske Kancellis udenrigske Afdeling. Almindelig Del. 3. Div. Sager vedr. hele Tiden til 1770. Chiffer."; p.10/309 eye-checked, shows a multi-column nomenclator/code table (numbered code groups against plaintext words, country headings incl. "Portugal", "Reich", "Frankreich", a cipher alphabet in the bottom-right corner) | Yes (the key; 309 pages, "Kopi, Scannet -- Arkivalieronline", freely viewable, no login) | High EV as a resource, not yet a solved pairing: the key collection is digitised and spans exactly the institution that also holds a named, dated, sender/recipient-identified run of *intercepted, presumably-still-undeciphered* cipher letters (Lagau, 1712) from the same chancery -- but the letters themselves are **not digitised** (one "pakke", "Original, Papir m.m.", order-only via Daisy, no Arkivalieronline link), so this is not a confirmed key-to-this-ciphertext match, only a same-creator, same-language(French)-plausible pairing worth a copy order. Next step for whoever takes this: order the Lagau "pakke" (Daisy heid 3245448) via REQUEST.md, then grep the 309-page Chiffer key volume for a France/Stockholm-specific table (it covers many countries in one volume) before assuming this exact key applies. Not found in QUEUE.md/CATALOG.md/LANDSCAPE.md/ciphers/ (grepped "Lagau", "Tyske Kancelli", "La Marck"), not in sources/wvo or sources/huygens, not in fresh shallow clones of both solver repos (one incidental substring false-positive checked and ruled out), no live ROOM.md claim. |
| VX-S02 | Alvin (Uppsala University Library digital platform), "Grön portfölj med chiffre-klaver" ("Green briefcase with cipher keys"), alvin-record:164482, shelfmark F 474, part of the Gustavianska samlingen (Gustav III's own collection, willed to the library, sealed 50 years after his 1792 death) | 1767-1770 samt flera odat. (+ several undated) | N/A here (the key portfolio itself, associated name Gustav III, king of Sweden 1746-1792); flagged undeciphered material below | N/A for this row (key portfolio). **Flag**: the same Alvin host, same "Gustavianska samlingen", record alvin-record:163862 ("Gustav III:s egenhändiga skrifter: 6. Memoarer och bidrag till Konungens egen historia. T. II. 1778-1792", shelfmark F 415) has a specifically-catalogued sub-item, "171. Chiffre från Konungens ungdom." ("171. Cipher from the King's youth."), undated, author "Gustav III... (author in his/her own hand)", format "Non digital + Digital" at the parent-record level -- i.e. catalogued as an actual cipher document, not a key, in the king's own hand | **(i) key/nomenclator collection**: 10 catalogued contents incl. "Clavis Positivae", a "Chiffres François" folder (itself containing "Clavis Resolutiva", a person/phrase nomenclator "N:1. La Princesse Albertine..." and a large A-Z alphabetic code table "100. A..."), "Chiffre François, de Mr de Lary", paired "Chiffre à Chiffrer"/"Chiffre à Déchiffrer. Entre S.E. et Moy 1767" key-pair (encipher/decipher tables between "S.E." and "Moy"), "A Chiffrer. 1770", plus 3 physical metal cipher-key plates (round- and square-holed grilles) in envelopes -- every page eye-checked this pass is a nomenclator table or code list, none is running ciphertext | `https://www.alvin-portal.org/alvin/view.jsf?pid=alvin-record:164482` (record); leaves tested directly via `https://www.alvin-portal.org/alvin/attachment/record/alvin-record:164482/ATTACHMENT-0016` (large A-Z alphabetic nomenclator, "100. A..." item) and `ATTACHMENT-0013` ("N:1. La Princesse Albertine..." numbered name/phrase list) and `ATTACHMENT-0005`/`ATTACHMENT-0015` (further numbered code lists); "All digitized materials in Alvin may be downloaded and used freely" (site's own copyright statement) | Yes (the key portfolio; confirmed by direct image fetch with plain curl, no login, no session needed) | High EV, unconfirmed this pass: item 171 "Chiffre från Konungens ungdom" is exactly the missing half (an actual cipher document, not a key, by the same person, in a collection with matching digitisation status) but this scout could **not** render its specific leaf -- Alvin's viewer nests it inside a collapsed tree branch of a 126-part described record, and the two obvious approaches (clicking the tree node directly, clicking its one plain search-result link) both timed out as "element not visible" (behind an un-expanded ancestor node or a lazy-render pane) despite `browser_fetch.js` correctly locating and typing/clicking through the *rest* of this session's Alvin navigation (F 474 above rendered fine the same way). This is a UI-navigation gap, not a negative -- the record's own metadata already states "Format: Non digital + Digital", so it is very likely digitised; the next worker should try Alvin's "See specifically described parts (126)" AJAX list on `alvin-record:163862`, or its IIIF endpoint if one exists, rather than the tree. Not found in QUEUE.md/CATALOG.md/LANDSCAPE.md/ciphers/ (grepped "Gustav III", "chiffre-klaver", "alvin-portal", "F 474", "F 415"), not in sources/wvo or sources/huygens, not in fresh shallow clones of both solver repos, no live ROOM.md claim. |

**Method and per-host findings (in brief order):**

- **Riksarkivet Sweden.** `sok.riksarkivet.se`'s own robots.txt disallows `/*sokord`, `/*AvanceradSok`, `/*bildvisning`, `/*FacettFilter` for *every* user agent (not just AI bots), which rules out its search UI and image viewer entirely, consistent with round 1's "client-rendered, no query URL found" finding. This pass located and documents, for the first time in this repository, the National Archives' actual public REST API: `https://data.riksarkivet.se/api/records?text=<term>&only_digitised_materials=true&limit=N` (found via `sok.riksarkivet.se/en/data-api/api` -> the `Riksarkivet/dataplattform` GitHub repo's wiki, `Search-API-records-english-version.md`; OpenAPI spec at SwaggerHub `Riksarkivet-Sweden/Riksarkivet-search`). No key needed. **Working only over `--http1.1`** -- plain curl to `/api/records` (any query) fails `SSL_ERROR_SYSCALL` on HTTP/2 roughly half the time even for the endpoint's own documented example query, while `/api/version` and `/api/status` work reliably either way; `--http1.1` fixed `/api/records` completely across the rest of this pass. Worth adding to the CLAUDE.md host table for the next Riksarkivet worker. Terms run: `chiffernyckel`, `chiffrerad`, `dechiffrerad`, `dechiffrering`, `i chiffer`, `chiffer`, `chiffreradt`, `chifferskrift`, `hemlig skrift chiffer` -- **zero new rows**: every relevant hit resolves to material already on file (the `SE/RA/202` "Chifferklaver" fonds, 1500t-1894, already exhaustively worked in `ciphers/oxenstierna-gustav-adolf-1632/NOTES.md` and QUEUE.md rows R9/R10/G4; the Celsing letter-drafts-with-key pair already on the board as `ciphers/ra-celsing-sillen-1755` and `ciphers/ra-celsing-dohsson-1779`, found by the prior LANE S scout of 24 Sept -- confirmed by directly reading both targets' NOTES.md, same shelfmarks `SE/RA/721512/II/II 1/II 1 B/4` and `SE/RA/721512/IV/IV 1/5`; `ciphers/ra-morner-welin` and `ciphers/ra-crusenstolpe-1809`, also already on file). No genuinely new Swedish target surfaced this pass despite a working new search route -- the vocabulary this lane's brief supplies has been thoroughly mined already via DECODE/Chifferklaver-based routes even though nobody had used this specific API before.
- **KB (Det Kongelige Bibliotek), Denmark.** `www.kb.dk`'s robots.txt blocks only its Drupal CMS `/search/`; the library's actual discovery layer (Primo, at `soeg.kb.dk`) has its own robots.txt disallowing `/discovery/search` and `/primaws` for *every* user agent -- its search is blocked for this pass by the site's own policy, not merely hard to drive. Not queried further. The static Oriental-manuscripts pages under `www5.kb.dk` are a different, narrow collection (not diplomatic correspondence) and were not swept given the budget already spent on the two rows above.
- **Arkivverket / Digitalarkivet, Norway.** `www.digitalarkivet.no`'s robots.txt names an extensive, current list of AI/LLM crawler user agents by name -- including `ClaudeBot`, `Claude-User`, `Claude-Web`, `Claude-SearchBot` and `anthropic-ai` -- each with `Disallow: /`; the generic `User-Agent: *` group underneath carries only a `Crawl-delay: 5`, no disallow, so this scout's literal descriptive UA string is not itself named. Given this session runs on Claude and the intent of that list is unambiguous, this pass treated the host as off-limits and did not query beyond the root reachability probe already logged by VX-SCEU (round 1). Flagged here rather than silently skipped so the next worker does not re-attempt it without deciding the same question.
- **National Library of Finland (`digi.kansalliskirjasto.fi`).** robots.txt disallows `/*search*`, `/*pdf*`, `/*page-*.xml`, `/*page-*.txt`, `/*.xlsx*` for every user agent -- its search UI and raw-text/PDF export routes are blocked by the site's own policy. This collection is also overwhelmingly newspapers/journals (OCR full-text), a lower-relevance target family for 17th-18th c. diplomatic cipher correspondence than the other Nordic hosts. Not queried beyond the round-1 root probe.
- **Alvin (`alvin-portal.org`, Uppsala/Swedish libraries).** robots.txt disallows only `/webstats/`, `/alvin/imageViewer.jsf`, `/iipsrv/` (the image tile server) and `/alvin/security/` -- its search and record pages are open. The plain search-box UI is a stateful JSF (JavaServer Faces) app (ViewState postback), not answerable by a bare curl GET; `tools/browser_fetch.js --type "#alvinForm\:searchField=<term>"` (fill-and-Enter) reliably reproduces a plain, repeatable-looking result URL (`resultList.jsf?...&query=<term>&searchType=EXTENDED...`), and record images fetch with plain curl afterward, no session needed (`attachment/record/alvin-record:<id>/ATTACHMENT-NNNN`, zero-padded four digits). Terms run: `chiffernyckel` (1 hit, VX-S02's F 474), `chiffre` (10 hits -- F 474 again, item 171 in F 415 flagged above, several ordinary Gustav III letters/memoranda with no cipher content, and one unrelated 1796 printed telegraph treatise by Edelcrantz), `chiffre lettre` (0 hits, phrase query too strict/AND-only). Not re-run for `dechiffrerad`/`cifferskrift`/Danish-Norwegian terms this pass (budget; Alvin's holdings are overwhelmingly Swedish/French per the hits seen).

**Drop-check performed for both rows:** shelfmarks, sender/recipient/collector names and the item titles grepped against QUEUE.md, CATALOG.md, LANDSCAPE.md, every `ciphers/*/NOTES.md`, `sources/wvo`, `sources/huygens`; fresh shallow clones of `dbourdeau/cyphersolver` and `aaymeloglu/unsolved-ciphers` (one incidental substring false-positive on "Lagau" inside unrelated German OCR text, checked and ruled out); last-30-lines and full-session ROOM.md claims (no collision).

**Per-host request counts:** `sok.riksarkivet.se` 1 (robots.txt only). `data.riksarkivet.se` ~28 (curl, `--http1.1`, includes retries after `SSL_ERROR_SYSCALL`; roughly half of raw attempts failed the TLS handshake and were retried once each, all successful queries >=1.5s apart). `raw.githubusercontent.com` 3 (wiki pages, not rate-limited, not a target host). `github.com` 2 fresh shallow clones (`Riksarkivet/dataplattform`, grepped only) + 2 more (`dbourdeau/cyphersolver`, `aaymeloglu/unsolved-ciphers`, drop-check greps only). `www.kb.dk` 1, `soeg.kb.dk` 1 (robots.txt only, both). `www.digitalarkivet.no` 0 this pass (round-1 probe only; robots-declined). `digi.kansalliskirjasto.fi` 0 this pass (round-1 probe only). `en.rigsarkivet.dk` 2. `daisy.rigsarkivet.dk` ~9 (1 plain curl + `browser_fetch.js` navigations, well-spaced). `soeg.rigsarkivet.dk` 1 (robots.txt only, then declined). `arkivalieronline.rigsarkivet.dk` 2 (`browser_fetch.js` page loads). `www.alvin-portal.org` ~20 (mix of `browser_fetch.js` searches/clicks and plain curl image fetches, >=1.5s apart). No DECODE, no credentials, no WebSearch beyond locating the two hosts' own official API/platform documentation (Riksarkivet's data-platform wiki, Rigsarkivet's "new search platform" announcement) -- both used only to find a working route on a host already in scope, not as a discovery channel of their own. No subagents (both allowed, neither needed).
...
| # | Inv. | Date | Place | Shelfmark | Sender → Recipient | Index wording (quoted) | Key route | Note | CSP check-solved (NX2-CSP, 26 Sept 2026) |
|---|---|---|---|---|---|---|---|---|---|
| 1 | 682 | 1478.12.03 | Nápoles | A-7, f.236 | Luis Despuig (Maestre de Montesa) → Reyes Católicos | "No existe el texto descifrado de esta carta." | none known — pre-dates Kolosova's 1521-27 corpus | different era, low priority | no CSP coverage (1478, before vol.I's 1485 start) -- not run |
| 2 | 890 | Julio 24 (Reyes Católicos era) | Nápoles | A-9, f.126-127 | Galcerán de Requesens, conde de Palamós → Fernando e Isabel | "Original, cuyo texto descifrado no existe a continuación." | none known — same pre-1521 era | different era, low priority | no CSP coverage (undated within the Reyes Catolicos era, before Kolosova's 1521-27 corpus) -- not run |
| 3 | 3090 | 1522.05.14 | Venecia | A-24, f.65-67 | Alonso Sánchez → Charles V | own entry bare "en cifra"; trailing note (misfiled by OCR boundary onto inv. 3091) says decipherment leaves torn out, and two Fernando Marín/Nájera letters (Apéndice B nos. 124-125) also lost | **Ko.9** (Kolosova p.350-380; Tomokiyo `AlonsoSanchez.htm` on file) | held pool — see rah-salazar-soria-sanchez-1524-28 | NOT CALENDARED -- CSP vol.II (Bergenroth) checked for 14 May 1522: entries jump 13->16->18 May, no Sanchez entry in the window |
| 4 | 4249 | 1524.04.14 | Roma | A-31, f.88-96 | Duke of Sessa (Luis Fernández) → Charles V (duplicate of 4248) | "...se nota arrancado el texto descifrado de estas cartas. Véase el Apéndice B, n2 16." | Ko.1 (Kolosova) | **check overlap with Bourdeau's `sessa1524`** (Signatura 9/28-9/34) before opening | IN PRINT -- CSP vol.II, "The Duke of Sessa, Imperial Ambassador in Rome, to the Emperor," Rome, 14 Apr 1524, "Spanish. Holograph partly in cipher. Contemporary deciphering. pp.18" (entry no. OCR-garbled, reads ~688/689) |
| 5 | 4670 | 1524.12.17 | Venecia | A-33, f.72-74 | Alonso Sánchez → Charles V | "...no están descifrados a continuación." | **Ko.9** | **held** — rah-salazar-soria-sanchez-1524-28 item 4 | NOT CALENDARED (NX2-GATE2, held item 4) -- stays open. RV: not found in either Rodríguez Villa work (NX2-RV, 26 Sept 2026) -- date predates both books' coverage |
| 6 | 5180 | 1525.09.21 | Génova | A-35, f.310-312 (+1 dup.) | Lope de Soria → Charles V | "...que no están descifradas a continuación." | **Ko.6** (p.284/307) or Ko.16 — unresolved which | **held** — rah-salazar-soria-sanchez-1524-28 item 1 | IN PRINT (NX2-GATE2, held item 1) -- CSP III pt.1 No.212 |
| 7 | 5503 | 1526.05.29 | Roma | A-37, f.324-327 | Duke of Sessa → Charles V (duplicate of 5502) | "...arrancadas algunas hojas que debieron contener el texto descifrado..."; also cross-refs Apéndice B n.107 (= stolen, not recoverable) | Ko.1 | **check overlap with `sessa1524`** | NOT CALENDARED -- CSP vol.III pt.1 checked for 29 May 1526: Sessa entries fall on 4/18/25/26 May only, none on the 29th; next entry 2 June |
| 8 | 5520+5521 | 1526.06.05 | Venecia | A-37, f.363-371 (orig+dup) | Alonso Sánchez → Charles V | "...por estar en cifra y no hallarse a continuación el texto descifrado, no se puede precisar cuáles serían [los asuntos]" — cataloguer could not even summarize the letter | **Ko.9** | not on file before this sweep, not in ciphers/ or held pool | NOT CALENDARED -- CSP vol.III pt.1's only 5 Jun 1526 entry is Lope de Soria writing from Genoa, referencing "Sanchez' despatches" from Venice, not a Sanchez letter itself; no Venice-dated 5 Jun entry found. RV: not found in *Italia...* (in coverage, no "5 junio" match); *Memorias* out of range (NX2-RV, 26 Sept 2026) |
| 9 | 5538 | 1526.06.13 | Venecia | A-37, f.419-422 | Alonso Sánchez → Charles V | "...no se hallan descifrados." | **Ko.9** | not on file before this sweep; adjacent D crib (inv. 5537) is for Lope de Soria, not this correspondent | IN PRINT -- CSP vol.III pt.1 No.459, "Alonso Sanchez, Imperial Ambassador in Venice, to the Emperor," Venice, 13 Jun 1526, shelfmark "M. Re. Ac. d. Hist. Salazar, A.37, ff.419-22" (exact match), (Cipher:)/(Common writing:) alternating, "Spanish. Original. Contemporary deciphering. pp.10" |
| 10 | 5540 | 1526.06.16 | Génova | A-37, f.428-429 | Lope de Soria → Charles V | "...sin descifrar." | **Ko.6**/Ko.16 | **held** — item 2; adjacent Caracciolo D pair (5541/5542) resolves S3, see above | NOT CALENDARED (NX2-GATE2, held item 2) -- stays open. RV: not found in *Italia...* (in coverage; nearest Soria letters are 10 and 28 June) (NX2-RV, 26 Sept 2026) |
| 11 | 5543 | 1526.06.18 | Venecia | A-37, f.438-440 | Alonso Sánchez → Charles V | "...cuyo texto descifrado no existe a continuación." | **Ko.9** | not on file before this sweep; "Publicada en parte por Rodríguez Villa, ob. cit. pp.133-134" — worth checking that print edition | NOT CALENDARED -- CSP vol.III pt.1 checked for 18 Jun 1526: the entries under that date are Jonglet/Simon de Tassis/Marquis del Guasto, not Sanchez; shelfmark f.438-440 not found anywhere in the volume. RV: checked exactly -- *Italia* pp.133-134 print a passage headed "13 junio 1526", not 5543's own 18 June; that date matches inv.5538 (already IN PRINT), not this entry -- the Índice's own citation looks misattached, not a confirmed print appearance; stays NOT CALENDARED (NX2-RV, 26 Sept 2026, see NOTES.md for detail) |
| 12 | 5545 | 1526.06.19 | Venecia | A-37, f.444-447 | Alonso Sánchez → Charles V | "...cuyo texto descifrado no existe a continuación. Es traslado de otra embiada..." | **Ko.9** | not on file before this sweep | NOT CALENDARED -- CSP vol.III pt.1 checked for 19 Jun 1526: the calendar jumps from 18 to 20 June with no Sanchez entry between; shelfmark f.444-447 not found anywhere in the volume. RV: not found in *Italia...* (in coverage; no 19 June entry) (NX2-RV, 26 Sept 2026) |
| 13 | 6120 | 1527.07.20 | Venecia | A-41, f.27-28 | Alonso Sánchez → Charles V | "...que no se encuentran descifrados a continuación." | **Ko.9** | not on file before this sweep | IN PRINT, flagged -- CSP III pt.2, "Alonso Sanchez ... to the Emperor," Venice, 20 Jul 1527, shelfmark "Salazar, A.41, f.29" (RAH gives f.27-28, off by 1-2 folios); footer "Spanish. Original. pp.4" carries no (Cipher:) marking at all, unlike rows 9/14/17/18 below -- date/sender/place match exactly but the cipher-passage status doesn't square with the RAH's own note; report as a probable match, not a settled one. RV: not found in *Memorias...* (in coverage; TOC has no Sánchez entry between 25 Jun and 23 Aug 1527); *Italia* ends 6 May 1527, out of range (NX2-RV, 26 Sept 2026) |
| 14 | **6122** | 1527.07.24 | Milán | A-41, f.33-37 | Marino Caracciolo → Charles V | "...con algunos párrafos en cifra que no se hallan descifrados a continuación." | **Ko.8** (p.338/348, Italian) | **(withdrawn by the CSP sweep below) strongest candidate**: genuinely undeciphered (unlike S3/inv.5541, same correspondent, interlinear-deciphered elsewhere in this collection — direct proof the key family is provable) | IN PRINT -- CSP III pt.2 No.122, "Prothonotary Caracciolo to the Emperor," Milan, 24 Jul 1527, (Cipher:) paragraph given in English, "Italian. Holograph partly in cipher. pp.9" (shelfmark column OCR-garbled/unreadable but sender+date+place match exactly); **reverses this row's own #1 ranking below** |
| 15 | 6124 | 1527.07.27 | Venecia | A-41, f.41-43 | Alonso Sánchez → Charles V | "...cuyo texto descifrado no existe a continuación." | **Ko.9** | not on file before this sweep; same-tomo D crib exists (inv. 6191, "Texto descifrado de las cartas 6188 y 6189", Alonso Sánchez, A-41) — not page-adjacent but same correspondent/legajo | IN PRINT, flagged -- CSP III pt.2, "Alonso Sanchez" ... to the Emperor (and to Soria), Venice, 27 Jul 1527, footer "Spanish. Original. pp.5" carries no (Cipher:) marking -- same caveat as row 13 (inv.6120). RV: not found in *Memorias...*, same window/method as row 13; *Italia* out of range (NX2-RV, 26 Sept 2026) |
| 16 | 6192 | Sin data (1527) | — | A-41, f.193-195 | Alonso Sánchez → **Martín de Ugarte** (not Charles V) | "Original, cuyo texto no se halla descifrado a continuación." | Ko.9, **unconfirmed for this addressee** — ciphers were often per correspondent-pair, not per-sender | not on file before this sweep; flag uncertainty before assuming Ko.9 applies | NOT CALENDARED -- "Ugarte" has zero hits anywhere in CSP vol.III pt.1 or pt.2 (both fetched whole, grepped in full). RV: "Ugarte" has zero hits in either Rodríguez Villa work either (every match is the common noun "lugarteniente") (NX2-RV, 26 Sept 2026) |
| 17 | 6331 | 1528.01.19 | Venecia | A-42, f.70-71 | Alonso Sánchez → Charles V | "...que no se encuentran descifrados a continuación." | **Ko.9** | not on file before this sweep; "Publicada, en parte, por Rodríguez Villa... página 362" | IN PRINT -- CSP III pt.2 No.292, "Alonso Sanchez, Imperial Ambassador at Venice, to the Emperor," Venice, 19 Jan 1528, several (Cipher:) sections, "Spanish. Original partly in cipher. Contemporary deciphering on separate sheet (f.72). pp.8" (RAH gives A-42 f.70-71 -- decipherment sheet at f.72 fits the Indice's own pattern of filing a decipherment as the very next folio) |
| 18 | 6364 | 1528.02.12 | Nápoles | A-42, f.143-148 | Hugo de Moncada → Charles V | "...con muchos párrafos en cifra que no se hallan descifrados." | **none confirmed** — Tomokiyo (`spanish2C.htm`, 8 June 2026 note) records a *different*, undeciphered Moncada cipher (BNE, 6 Oct 1524) that matches none of Ko.1-17; check whether this RAH item is the same key before assuming it is untestable | not on file before this sweep; low priority without a key | IN PRINT (via Gayangos's own 19th-c solving, not a period key) -- CSP III pt.2 No.342, "Don Ugo de Moncada to the Emperor," Naples, 12 Feb 1528, full narrative content given in English despite the footer "Spanish. Original almost entirely written in cipher. No deciphering appended. Duplicate. pp.12" -- same pattern as row 20 (inv.6502); does not identify which cipher key was used |
| 19 | 6417 | 1528.04.14 | Mirandola | A-42, f.243-244 | Lope de Soria → Charles V | "...que están sin descifrar." | **Ko.6**/Ko.16 | **held** — item 3 | IN PRINT (NX2-GATE2, held item 3) -- CSP III pt.2 No.399 |
| 20 | 6502 | 1528.06.17 | Venecia | A-42, f.443 | Alonso Sánchez → **Mercurino de Gattinara** (chancellor, not the Emperor) | "...que no están descifradas a continuación." | Ko.9, **unconfirmed for this addressee** | **held** — item 5; direct D crib immediately adjacent (inv. 6501, "Texto descifrado de los párrafos en cifra de los tres documentos anteriores", same correspondent — this is S2, held) | IN PRINT (NX2-GATE2, held item 5, Gayangos's own solving) -- CSP III pt.2 No.462 |
| 21 | 13760 | 1618.08.07 | Nápoles | A-84, f.114 | Juan de Dávila → Diego Sarmiento de Acuña (Gondomar) | "...con frases en cifra que no están descifradas." | none known — Gondomar-era, different key family | = SCOUT-OWN-4's S4, single letter, low priority | no CSP coverage (1618, after vol.XIII's 1558 end) -- not run |
| 22 | 15232 | 1656.07.02 | San Sebastián | A-97, f.143-144 | Baron de Vatteville → Luis Méndez de Haro | "...un párrafo en cifra que está sin descifrar." | none known directly; same office/decade as the Gayangos crib (row above, S5) | = S5a; small (1 paragraph) | no CSP coverage (1656) -- not run |
| 23 | 15570 | 1655.10.23 | Roma | A-101, f.70-71 | Cardinal Giovanni Carlo de' Medici → Luis Méndez de Haro | "...con un párrafo en cifra que no se halla descifrado a continuación." | none known; entry itself notes Haro had just *sent* this cardinal a fresh cipher ("le agradece el envío de la cifra para su correspondencia") — likely a distinct, newly-issued key, not shared with Vatteville's | not on file before this sweep; small (1 paragraph); same Haro network | no CSP coverage (1655) -- not run |
| 24 | 15670 | 1648.04.20 | Nápoles | A-103, f.22-23 | Carlos Doria y Doria Carreto → Juan José de Austria | Gayangos's own marginal/interlinear decipherment (D, not U) — see S5 above | period, Gayangos (this same item) | = S5b, the crib itself, held | no CSP coverage (1648) -- not run (this is the D-graded crib itself, not a U row) |
| 25 | 16231 | 1661.09.11 | Badajoz | A-107, f.193 | Juan José de Austria → Luis Méndez de Haro | "Original, en cifra. No se halla el texto descifrado a continuación." | none known — same addressee (Haro) as S5's crib but a different decade/front (Badajoz/Portugal war, 1661, vs Naples 1648); same-key assumption unconfirmed | not on file before this sweep | no CSP coverage (1661) -- not run |
...
1. **Row 14 (Caracciolo, inv. 6122)** — published key (Ko.8), and the *same correspondent's* cipher
   ...
   5541/5542). Strongest single candidate this session (withdrawn by the CSP sweep below); not yet in any `ciphers/` folder.
2. **Rows 5, 6, 19, 20 + the row-20 crib (inv. 6501/S2)** — the existing `held`,
   ...
   named in its own NOTES.md); rows 8, 9, 11, 12, 13, 15, 17 (all Alonso Sánchez items not on file before, same
...
### CSP check-solved sweep (NX2-CSP, 26 Sept 2026) — corrects the ranking above

**Method.** Every U/U+D row dated 1485-1558 not already checked by NX2-GATE2 (rows 5, 6, 10, 19, 20 — the
original held pool) was checked page-by-page against CSP Spain, same method as NX2-GATE2 (fetch once, grep
by correspondent and date, read every matching entry by hand): CSP vol.II (Bergenroth, archive.org
`bub_gb_ZoY9AAAAcAAJ_djvu.txt`, covers into 1526) for the 1522/1524 rows, vol.III pt.1 (Gayangos,
`calendarofletter0003pasc_djvu.txt`, ends ~Dec 1526) for the mid-1526 rows, vol.III pt.2 (Gayangos,
`calendarorleters0003vari_djvu.txt`, 1527-1529) for the 1527-1528 rows. Rows dated after 1558 or before 1485
(rows 1, 2, 21-25) are out of CSP's coverage and were not searched, per this job's brief.

**Result: 4 more rows are already in print, one of them reversing this section's own #1 ranking.**
Row 14 (inv. 6122, Marino Caracciolo → the Emperor, Milan, 24 Jul 1527) — called "the strongest single
candidate this session" above — is calendared in CSP vol.III pt.2 (No. 122) with its `(Cipher:)` paragraph
given in English ("Italian. Holograph partly in cipher. pp. 9"): **its plaintext is already published, and
it is not a cryptanalysis candidate.** Rows 4 (inv. 4249, Duke of Sessa, Rome, 14 Apr 1524, CSP vol.II,
"Contemporary deciphering," pp.18), 9 (inv. 5538, Alonso Sánchez, Venice, 13 Jun 1526, CSP III pt.1 No.459,
shelfmark match exact, "Contemporary deciphering," pp.10) and 17 (inv. 6331, Alonso Sánchez, Venice, 19 Jan
1528, CSP III pt.2 No.292, "Contemporary deciphering on separate sheet (f.72)," pp.8) are likewise already
published with period decipherments. Row 18 (inv. 6364, Don Ugo de Moncada, Naples, 12 Feb 1528, CSP III
pt.2 No.342) is published too, but by Gayangos's own 19th-century solving — the footer reads "No deciphering
appended," yet the full narrative content is given in English anyway, the same pattern as row 20's already-
known case. Two more (rows 13 and 15, inv. 6120 and 6124, both Alonso Sánchez, Venice, 20 and 27 Jul 1527)
match a CSP entry exactly on sender/recipient/place/date, but the CSP footer carries no `(Cipher:)` marking
at all ("Spanish. Original. pp.4" / "pp.5") — inconsistent with the RAH's own "sin descifrar" note for these
two, so flagged as a **probable but unconfirmed match**, not a settled one; see the table column for the
exact wording. Full detail and shelfmark caveats (OCR-garbled shelfmark columns for rows 14/18, a folio-
number mismatch for row 13) are in the table's own column above, not repeated here.

**8 of the remaining rows are genuinely not calendared** (3, 5, 7, 8, 10, 11, 12, 16 — 5 and 10 already
known from NX2-GATE2, the other six checked this session), each checked page-by-page in its date window with a
negative result recorded in the table above (rule 10: "not found in CSP Spain vol.<X>, searched by
page-by-page grep of the fetched volume for the correspondent and date, 26 Sept 2026" — never "unread" or
"new"). **7 rows have no CSP coverage at all** (1, 2, before vol.I's 1485 start or undated; 21-25, after
vol.XIII's 1558 end) and were not searched, per this job's own scope.

**Net effect: of the 18 rows CSP could possibly cover, 10 are already in print (2 of those flagged as
probable, not confirmed) and 8 are genuine negatives.** This is the same pattern NX2-GATE2 found in the
original five-item pool (3 of 5 already in print) at a larger scale: the RAH Índice's own "sin descifrar"/
"no se hallan descifrados" cataloguing describes the manuscript's own state, not whether Gayangos separately
published a translation from a since-lost decipherment sheet or solved the cipher himself. **Ranking item 1
above is withdrawn** (row 14 is in print, not a candidate); rankings 2-6 are otherwise unaffected by this
sweep except that rows 8, 9, 11, 12, 13, 15, 17 (item 2's "should be folded into that folder" list) now split
further: 9 and 17 are in print, 13 and 15 are flagged probable-in-print, and only 8, 11, 12 (plus row 16, a
different addressee) remain genuinely open Sánchez rows in that run. REQUEST.md updated accordingly (rows
6122 and 6124 moved out of "Target" status).

Hosts: archive.org 3 fetches (one per volume, 1.5+ s apart, all already-identified ids from NX2-GATE2's
NOTES.md, no new identifiers looked up). No subagents, no logins, no image opened, no novelty wording.

### Rodríguez Villa check (NX2-RV, 26 Sept 2026) — the final cheap filter before the copy order

**Method and result.** Checked the six rows still NOT CALENDARED after the CSP sweep (5, 8, 10, 11, 12, 16 —
inv. 4670, 5520/5521, 5540, 5543, 5545, 6192) plus the two CSP-flagged probable rows (13, 15 — inv. 6120, 6124)
against the two Rodríguez Villa works the Índice's own "ob. cit." citations resolve to: *Italia desde la
batalla de Pavía hasta el Saco de Roma* (Madrid, 1885, covers Jan 1525-6 May 1527) and *Memorias para la
historia del asalto y saqueo de Roma en 1527* (Madrid, 1875, covers Sept 1526-March 1528); both fetched once
from archive.org and grepped by date/correspondent, full detail and per-row search log in
`ciphers/rah-salazar-soria-sanchez-1524-28/NOTES.md`. **All eight: not found.** One flag worth carrying
forward: row 11's own Índice citation ("Publicada en parte por Rodríguez Villa, ob. cit. pp.133-134") does
not actually describe inv. 5543 — pp.133-134 print a passage dated 13 June 1526, not 5543's own 18 June; that
date belongs to inv. 5538 (already IN PRINT via CSP, row 9). This is a citation mismatch in the Índice, not a
confirmation that 5543 is in print — row 11 stays NOT CALENDARED. This closes the cheap-test ladder for this
pool: CSP settled 10 of 18 rows in range, Rodríguez Villa adds no further settlements, only the one
mismatch flag. Six rows (5, 8, 10, 11, 12, 16) remain genuinely open and are the copy-order's core ask;
rows 13 and 15 stay flagged probable-but-unconfirmed, unchanged by this pass.

...
corrects the catalogue, and checks the reading against the printed translation. **r22 is a proposed repair, not
applied** -- see REPAIRS.md; ciphertext.txt still transcribes "palatinus" (rule: never silently repaired).
...
case, and <...> marks word signs. [..] marks tokens unread or M. Ten positions below (r04, r06 twice, r14 twice,
r18, r23, r29, v02, v09) now print the interlinear gloss's own word rather than key.tsv's mechanically-applied
value; see "Exceptions (per-position gloss overrides)" below for the full list and reasons. They were rendered
with brackets or `[sic]` before ASSIGNMENTS row 25 (23 Sept 2026); that is no longer needed, since check.py now
applies the gloss at those positions directly and grades them H.
...
FILIUS <DUX> <LUNEBURG> tractatum cum patre meo ad patrem, et EGO IOCHIN UT PROPOSUI <MARCHIO> COMMISSIONEM
...
[?]ET UNUS ARTICULUS IMPONETUR UT NULLUS ALIUS IN ISTUD FEDUS ACCEPTARETUR NISI MAIORE PARTE CONFEDERATORUM
CONSENTIENTE, ET QUEMCUMQUE MAIOR PARS ADMITTERET aut quoniam [ß] CONSENTIRET, UT MINOR PARS ADMITTERE ET
CONSENTIRE TENERETUR. Deinde potest palatinus TRACTARI CUM HIS QUI NON dum habent speciale FEDUS AUT singulare
AMICITIAM CUM <CHRISTIANISSIMO>, UT SPECIALE FEDUS FIAT aut amicitia; a[?] tandem ubi SECURUM SIT [g] quod LONGE
...
f.28v: Nos omnino speramus quod firmiter ut cito faciemus MAIOREM LIGAM [g] ET CONFEDERATIONEM IN FAUOREM
...
FEDUS [g] INTER NOS FACTUM SIT, ac tempus erit ut <CHRISTIANISSIMUS> INGREDIATUR; deinde <CHRISTIANISSIMUS> poterit
...
## Exceptions (per-position gloss overrides, ASSIGNMENTS row 25, 23 September 2026)

exceptions.tsv lists ten (line, position) pairs where the word the glossator actually wrote over a token
disagrees with key.tsv's dominant value for that symbol -- a minority vote among the votes in
key_from_gloss.tsv (see "Self-consistency" above: 9 single-letter-type minority votes, plus the r04 word-sign
case that the Grading table below already carried as M). check.py now applies exceptions.tsv as a per-position
override, so reading.txt prints the gloss's own word at these ten spots instead of the symbol's majority value.
Every exception is graded **H** (rule 4: read from a key source, the gloss on the document, at that exact
position), since it is literally the gloss's own reading there, not the solver's inference.

| line:pos | token | key.tsv value | gloss value (now in reading.txt) | reason |
|---|---|---|---|---|
| r04:25 | PW | =Mekleburg | =dux (<DUX>) | word-sign token glossed 'dux' at this one occurrence, not Mekleburg; the second, adversarial audit's own alignment table already inferred "[ducis]" here from the printed French "filz du duc de Lunembourg" (AUDIT.md section "It is this letter"), so this exception also corrects a substantive misreading, not only a spelling |
| r06:12 | U | n | m | gloss writes m where the cipher spells n (commissionem, medieval -cio-/-tio- and n/m spelling pattern) |
| r06:17 | D | m | i | single-token encipherer's slip: D stands where commissionem has i, not m |
| r14:21 | Po | t | s | single-token slip: Po stands where articulus has s, not t |
| r14:23 | U | n | m | gloss writes m where the cipher spells n (imponetur) |
| r18:4 | P | s | t | single-token slip: P stands where consentiente has t, not s |
| r23:17 | L | c | t | gloss writes t where the cipher spells c (amicitiam, -cio-/-tio- pattern) |
| r29:1 | L | c | r | glossator read 'rum' over L-cc-[blot]; L stands for r here, not c (the blot itself, r29:3, has no key entry and stays unread) |
| v02:18 | L | c | t | gloss writes t where the cipher spells c (confederationem, -cio-/-tio- pattern) |
| v09:17 | q | e | n | glossator wrote 'nos' over q-Y-P here (the key reads 'eos'); he glossed the same three-sign group 'eos' at r12 |

**Grade counts changed by this table** (all ten were S before; nine were graded S from the gloss-derived key
applied elsewhere, one -- r04:25 PW -- was graded M as a word sign whose own gloss disagreed): H rises from 630
to 640, S falls from 33 to 24, M falls from 18 to 17. Unread stays 6 -- the two no-key-entry positions the
gloss also touches, Y~ (r04:15) and the r29:3 blot, are outside this table's scope (there is no key reading
for check.py to except there) and are unchanged.
...
| grade | tokens | what |
|---|---|---|
| H | 640 | under a gloss word that aligns with it, with the gloss letter or word equal to the key value, or a per-position exception (exceptions.tsv, 10 tokens, ASSIGNMENTS row 25) |
| S | 24 | not so glossed, read with a gloss-derived value (the key applied; control 99.8-100%) |
| M | 17 | g (13) and o (3), null by the search only; SX (Saxonie, from the gloss 'saxo') |
...
**Caveat on line r04, resolved (23 September 2026).** reading.txt previously regenerated r04 by applying
key.tsv's word-sign globally, printing MEKLEBURG where the gloss at that place reads "dux". check.py now applies

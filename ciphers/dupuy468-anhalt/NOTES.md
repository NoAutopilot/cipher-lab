...
**Caveat on line r04 (23 September 2026).** reading.txt is regenerated by applying key.tsv globally, so r04 reads the Mecklenburg word-sign as MEKLEBURG where the gloss at that place reads "dux" (key_from_gloss.tsv records the exception). Quote r04 only with the gloss reading; the regenerated line is not the identity passage's text. A per-position exception table in check.py is queued (ASSIGNMENTS row 25).

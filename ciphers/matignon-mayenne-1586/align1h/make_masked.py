#!/usr/bin/env python3
"""bMAT1H (copied unchanged from align1g; bMAT1G copied from align1e/make_masked.py; reads f78_corrected_stream.txt + Bourdeau f79_cipher.txt; ?-signs shown as <?>): build masked copies of the f.78v/f.79r cipher streams (Bourdeau's transcriptions,
...
for r in csv.reader(open(os.path.join(here, '..', 'key.tsv')), delimiter='\t'):
    if r and r[0] != 'code' and not r[0].startswith('#'):
        key[r[0]] = (r[1], r[2])
leaves = {f: [l.split() for l in open(os.path.join(here, {'f78': 'f78_corrected_stream.txt', 'f79': 'f79_cipher.txt'}[f])) if l.strip()] for f in ('f78', 'f79')}
toks = [t for f in leaves for l in leaves[f] for t in l if not t.startswith('?')]
...
for f in leaves:
    ...
    for i, l in enumerate(leaves[f]):
        w = []
        for t in l:
            if t.startswith('?'): w.append('<?>')
            elif t in lab: w.append('<' + lab[t] + '>')

...
## bMALX: cribs from record 509 and the clear clauses (26 Sept 2026)

Intake gate re-run: `python3 tools/intake_gate_check.py malsburg-hessen-1636` -> `partial` (line 1), exit 0.

**Question.** ff.16/23/24/28's clear clauses gave crib context but not a single named-code/gloss
pairing (bMALG/bMAL28B). Record 509 (ff.32-33, "around 1637") is different in kind: it is not a
solid cipher block but code groups written inside otherwise clear German/French prose (extent.tsv),
the only place in this fond where individual cipher groups sit directly against ordinary running
text on both sides. This job reads ff.32/33 and asks whether any code's syntactic slot is
consistent enough, across repeats, to be worth a nomenclator campaign. Not an interlinear
decipherment (no clear text repeats the cipher elsewhere) -- `tools/interlinear_align.py`'s
hard-EM loop needs a (group, span) pairing against known-elsewhere plaintext, which this leaf does
not have; not used, per the brief's own note.

**Fetch.** f.32/f.33 regenerated byte-identical to `images/manifest.json`'s recorded sha1
(`48e1af58...`, `6919ab57...`) via `regen_images.sh page <folio>` -- 2 requests to
`api.hcportal.eu`, both HTTP 200, 1.5s apart, none retried (images had been deleted 26 Sept by
bMALS's folder-shrink pass; byte-identical regen confirms the shrink's own regen-test claim held).
Neither leaf needed resizing (2114x2512, 2164x2516, both under 2400px longest side already).

**Crop step.** `tools/iiif_lines.py --lines-per-crop 3` was tried first (per the brief) but its
tight per-line bands (pitch 44-48px on this leaf) clipped ascenders/descenders mid-line even at
3 lines per crop -- confirmed on the debug overlay and on a direct read attempt that silently
skipped a whole manuscript line at a band boundary. Replaced with a small script cutting
overlapping 380px-tall native-resolution bands (60px overlap, `crops/{0032,0033}/manifest.json`
records box + source_file + sha1 per band, reproducible) -- each band is a fraction of the page,
never the full leaf, so the "subagent gets crop paths only" rule is still met. Flagged here per
the brief's "note in one line if you consider it" instruction; this is a real deviation from the
named tool, not an oversight.

**Passes.** Pass A: one Sonnet subagent, blind (no NOTES.md, no other transcription), given only
the band crop paths and the bMALG glyph-convention hint (dotted cursive stroke = digit 1, not
letter i); wrote `transcription/{0032,0033}_passA.tsv`, 219 code-token rows at token grain
(immediate left/right neighbour). Pass B: this worker, blind to pass A (written before pass A's
report arrived), read the same bands independently at native and 3x-zoomed resolution into
`transcription/{0032,0033}_passB_lines.txt` (full per-manuscript-line token sequences, not
token-by-token rows). The two passes agree closely on the code *inventory* and *structure*
(header "Extrait 70.56.10.28. D.39.27: 91.79.24.16.30. schreibt auf 115.S.13.15.35.22.[78/98].63."
read near-identically by both; "248." starting the body; the "weil"/"als"/"nach" function-word
boundaries; the "X.X. oder [mark]. nach G.G. genannt" / "mit G.G. oder [mark]" naming pattern
present on both leaves) with genuine digit-level disagreement on a handful of values (7 vs 9,
consistent with bMALG's own documented ambiguity; second mark after G.G. read as "K.K." by this
worker vs "A.A."/"GG." by pass A) -- not token-by-token reconciled given the job's cap/box (each
pass stands as its own LOW CONFIDENCE read, same convention as clear_0023.txt/clear_0024.txt; do
not cite either as H/C grade). `cribs.tsv` (222 rows) is built from pass A's finer token grain
(`cribs_build2.py`) plus 4 rows sourced from bMAL28B's already-published f.28 clear-clause crib
(the only ff.16/23/24/28 file that pairs an actual code value with adjacent clear words; f.16/23/24
each only say "cipher follows/precedes" without transcribing the adjacent code itself, so no row
is addable from them without a fresh read of those runs -- one-line follow-up suggestion, out of
this job's scope).

**Control (rule 3, must be able to fail differently).** Candidate class per occurrence
(person/place/title/number-date/function word/noun/verb/unclear) assigned by a mechanical
nearest-bounding-word rule (`cribs_build2.py:guess_class` -- an article/preposition touching the
code -> noun; a naming verb -> person; a conjunction/verb on both/one side with nothing else ->
function word; otherwise unclear), applied uniformly in leaf/line reading order rather than sorted
by code, so the classifier itself cannot drift toward making a code's repeats agree (the brief's
"without looking at the code's other occurrences" instruction, made bias-proof by using a fixed
rule instead of per-row human judgement on 222 low-confidence rows). Full numbers in
HYPOTHESES.md's "bMALX crib consistency" section: **real overall class-agreement 0.812 vs shuffle
(N=1000) mean 0.780, p95 0.836 -- real sits inside the shuffle p95, a TIE, non-discriminating, not
a pass.** Per-class (the AX-NAMES caveat: a blended figure dominated by one class inflates the
shuffle floor toward it): `function word` real 0.062 vs shuffle p95 0.081 (below); `noun` real
0.000 vs shuffle mean 0.017 (below); `unclear` carries 88% of the rows and does almost all of the
blended weight. No class clears its own shuffle p95. **0 codes have a consistent candidate WORD
across 2+ occurrences** (no word, only class, was ever assigned -- this hand does not support
forcing a specific word at LOW CONFIDENCE, rule 2) -- well under the orchestrator's 3+ threshold
for opening a nomenclator family from this evidence.

**System-match (step 6).** 54 of 61 distinct numeric codes read in this crib pass (88.5%) occur in
`pool/pooled.tsv`'s 173-value sign set, `pool_match.py`; the shared values skew toward the pool's
own higher-frequency ranks (top shared rank 1, median shared rank ~30 of 173). The 7 codes NOT in
the pool -- 115, 118, 123, 126, 133, 155, 248 -- are all >=100, outside the pool's established
2-digit-dominant range (bMALN's `pool_numeric_0_99` cut drops the same tier). Reading: 509 draws
on the same code inventory as the rest of the pool (same number range and forms), not a separate
system -- but this job finds no crib-level signal for what any individual code, or the G.G./K.K.
(or A.A.) naming pair, actually stands for.

**Verdict.** Status stays `partial` (rule 5): a control-backed non-test (TIE), not a negative, and
not a positive -- no nomenclator family opened on this evidence (0 consistent words, class-level
tie). The G.G./[K.K. or A.A.] alternate-codename pattern (found independently on both leaves, by
both passes) is the single most suggestive lead in this job's data and the natural next step: a
token-level reconciliation of that specific 4-6-word span on both leaves (not a full-leaf
reconciliation) would settle the second mark and confirm whether "genannt"/"oder" really bracket
a two-name gloss, before spending a family_run.py campaign on it.

Files: `images/{hstam_4_h_1411_0032,0033}.jpg`, `images/manifest.json`, `crops/{0032,0033}/**`,
`transcription/{0032,0033}_passA.tsv`, `transcription/{0032,0033}_passB_lines.txt`, `cribs.tsv`,
`cribs_build.py` (superseded first attempt, kept for the record), `cribs_build2.py`,

...
## Reconciliation of the Raince letter, ff.28r-29v (23-24 Sept 2026)

Status unchanged: open. Reconciliation worker, 23 Sept 2026 23:18 to 24 Sept 2026 00:05 UTC. Full
account in reconciliation.md.

- **Output.** `ciphertext.txt` now holds the reconciled sign sequence of Raince to Madame: 5,725
  tokens, 50 sign types, 365 marked uncertain, 105 physical lines. It is regenerated by
  `glyphs/build.py`; `--check` exits 1 if stale. Also: `inventory.tsv`, `glyphs/contact_sheet.jpg`,
  and `glyphs/overrides.tsv` (127 signs settled by eye).
- **Moved, unchanged.** The earlier f.20 (Carpi) partial reading moved to `ciphertext_f20_carpi.txt`,
  and its reconciliation note to `reconciliation_f20_carpi.md`.
- **Gate not clean.**
  - Pass A (4,797) is 19.3 % below the final count. It never read the right-hand crop segments of
    f28r L07-L21 and f28v L02-L13.
  - Pass B is within 3 % once its overlap double-reads are removed.
  - 50 types exceed 40: about 22 rare signs are distinct on the page.
- **Crop gap found.** The line crops stop short of the writing at the gutter: f28v at x=4300 (ink to
  about 4390) and f29v at x=4150 (ink to about 4260). About 70 signs were visible to neither pass.
  Two IIIF region strips were fetched (images/gutter/, 2 Gallica requests) and are used by the
  segmentation. A re-cut of those crops would let a human pass see them.
- **Where the passes under-split.**
  - Neither pass separates q (ball on stem) from ρ (tail curling left). The image does.
  - Pass A also merged ψ with the arrow sign and ℓ with Λ.
  - Pass B changed its codes between f28 and f29.
- **Folder size.** Committed images stay under 30 MB (29.4 MB). The folder as a whole is about
  31.3 MB with the data and scripts. The f.20 full-resolution copies (images/img/*f23*/*f24*_full.jpg,
  9.5 MB) could be replaced by their IIIF URLs if the orchestrator wants the folder under 30 MB; not
  done here, since they belong to the other letter.

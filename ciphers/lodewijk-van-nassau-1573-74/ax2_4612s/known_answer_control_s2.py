#!/usr/bin/env python3
"""AX2-4612S2: known-answer control for tools/key_repair.py's fixed (--objective excess,
--no-null-below 121) behaviour, 3 seeds. Per the AX2-4612S2 job brief: pick k=8 codes total (used
>=5 times in ax4612tr/ciphertext_5811_cut833.tsv under key_full.tsv, whose current key_full value is
a plain single letter) and perturb all 8 -- 6 of them (3 pairs) get their letter values swapped
pairwise, and 2 of them (the H-S shape: a code that might really stand for a syllable) get a French
bigram value that is not their true letter -- then run tools/key_repair.py's repair() from this
perturbed key on the same 5811 cut and compare the repaired key to the untouched key_full: recovery
(does a code's repaired value match its true key_full letter?) is reported for all 8 together (the
brief's own gate denominator) AND for the 2 bigram-hidden codes alone (the brief's own 'report those
two separately'); false changes are counted on the other, untouched codes."""
import os
import random
import sys

...
def outputs():
    ...
    for code, c in sorted(pairs.items(), key=lambda kv: (-sum(kv[1].values()), kv[0])):
        ...
        rows.append(f"{code}\t{v}\tC\tinterlinear plaintext\t{n} aligned{'; also aligned to ' + other if other else ''}\n")
    out['key.tsv'] = ('# Key read from the note\'s own interlinear plaintext (grade C: known plaintext on the leaf, LANE W audit 24 Sept 2026); generated by build_align.py\n'

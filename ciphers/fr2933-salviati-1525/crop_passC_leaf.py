#!/usr/bin/env python3
"""Pass-C crops, any leaf (bSALC, 26 Sept 2026; generalised from L1c's crop_passC.py): usage crop_passC_leaf.py <leaf>.
Original description: for each (line,pos) in recon_box_f55v/disagreements.tsv, crop the box (keyed off the
...
with open(f'{LEAF}_boxes.tsv') as f:
...
im = Image.open(f'glyphs/crops/{LEAF}.png')
W, H = im.size

os.makedirs(OUT, exist_ok=True)

rows = list(csv.DictReader(open(f'recon_box_{LEAF}/disagreements.tsv'), delimiter='\t'))
manifest = []
for r in rows:
    line = int(r['line'])
    pos = float(r['pos'])
    ...
    fname = f'{OUT}/L{line}_P{r["pos"]}.png'
    crop.save(fname)

#!/usr/bin/env python3
"""Pass-C crops: for each (line,pos) in recon_box_f56r/disagreements.tsv, crop the box (keyed off the
integer position in f56r_boxes.tsv -- a split position like 8.5 shares its parent box, e.g. pos 8) at 5x
zoom from glyphs/crops/f56r.png, padded to include the immediate left/right neighbour boxes on the same
line for context. Writes passC_crops_f56r/L<line>_P<pos>.png. Blind script: reads only line/pos, never the
...
im = Image.open('glyphs/crops/f56r.png')
W, H = im.size

os.makedirs('passC_crops_f56r', exist_ok=True)

rows = list(csv.DictReader(open('recon_box_f56r/disagreements.tsv'), delimiter='\t'))
manifest = []
for r in rows:
    line = int(r['line'])
    pos = float(r['pos'])
    ...
    fname = f'passC_crops_f56r/L{line}_P{r["pos"]}.png'
    crop.save(fname)
    manifest.append((line, r['pos'], fname, f'box x{x},y{y},w{w},h{h} parent_pos={bp}'))

with open('passC_crops_f56r/manifest.tsv', 'w') as f:
    f.write('line\tpos\tfile\tnote\n')
    for row in manifest:
        f.write('\t'.join(str(c) for c in row) + '\n')

print(f'{len(rows)} disagreement rows -> passC_crops_f56r/, manifest passC_crops_f56r/manifest.tsv')

#!/usr/bin/env python3
"""Build ciphertext_f56r.tsv (line,pos,code,marks,grade) exactly as ciphertext_f55r.tsv: every
...
by recon_box_f56r/settled.tsv (2-of-3 majority with pass C, or a reasoned arbitration on a 3-way split)."""
import csv
...
A = load('passA_f56r.tsv')
B = load('passB_f56r.tsv')
settled = load('recon_box_f56r/settled.tsv')
...
with open('ciphertext_f56r.tsv', 'w') as f:
...
print(f'{len(out)} rows -> ciphertext_f56r.tsv (AB {n_ab}, settled {n_settled})')

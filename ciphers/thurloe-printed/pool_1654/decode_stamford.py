...
def decode_p4(lines, rows):
    units, a = [], A.SPANS["P4"]["cipher"][0]
    for b0, b1 in A.P4_COLUMN_BLOCKS + [(A.SPANS["P4"]["cipher"][1] + 1, None)]:
        units += A.tokenize(A.clean_lines(lines, a, b0 - 1))
        if b1:
            units += A.tokenize(A.clean_lines(lines, b0, b1), strict=True)
            a = b1 + 1
    toks, counts = [], Counter()
    for u in units:
        if u[0] == "W":

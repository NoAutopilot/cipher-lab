...
en18 corpus (ARM-EN18, 26 Sept 2026; CLAUDE.md rule 3 fold-count amendment, same method as
tools/data/en/holdout_check.py and tools/data/es17c/holdout_check.py). N defaults to 1000/1500
letters -- about the length of a 369-code-group decode on the armstrong-madison-1808 target,
which is what specs/armstrong-madison-1808.json's judge block needs checked, not the 200/500
used for LANG_CORPORA["en"]'s own short-ciphertext specs.

For each file, build an NgramModel from the OTHER five files only, draw SAMPLES windows of N
letters from the held-out file (never used to build that model), and for each window:
  - false negative: the real window fails judge()'s PASS condition (score <= real_p05, using
    thresholds computed from the training model's own controls).
  - false positive (null): the SAME window, letter-shuffled, clears both thresholds (score >
    null_p99 and score > real_p05).
"""
import argparse
import random
import sys
...
DEFAULT_FILES = [
    "writingsjamesmo02unkngoog.txt.gz",
    "writingsjamesmo11monrgoog.txt.gz",
    "writingsalbertg01gallgoog.txt.gz",
    "writingsofjamesm0007unse_s2a1.txt.gz",
    "writingsofjamesm0008unse.txt.gz",
    "writingsofthomas09jeffiala.txt.gz",
]
NS = [1000, 1500]
SAMPLES = 200
...
def run(files, ns=NS, samples=SAMPLES):
    ...
    for N in ns:
        ...
        summary[N] = (total_fn, total, spread, rows)
    return summary

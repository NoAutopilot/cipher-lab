...
5. For a hit that could print, calendar or discuss the letter itself, open the article IN JSTOR'S ONLINE PAGE VIEWER and read it there (online reading is unlimited on JPASS and 100 a month on a free account); never click Download PDF (10 a month on JPASS, and not needed); only for such hits and add to that target's AUDIT.md a section `## JSTOR (owner's machine, <date>)` with the citation and one sentence on what it says. Change no N-class.

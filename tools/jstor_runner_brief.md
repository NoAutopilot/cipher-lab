...
5. For a hit that could print, calendar or discuss the letter itself, open the article (100 reads a month; only for such hits) and add to that target's AUDIT.md a section `## JSTOR (owner's machine, <date>)` with the citation and one sentence on what it says. Change no N-class.

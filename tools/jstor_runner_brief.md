...
5. For a hit that could print, calendar or discuss the letter itself, open the article IN JSTOR'S ONLINE PAGE VIEWER and read it there (JPASS monthly on the owner's account from 24 Sept 2026: unlimited online reading); never click Download PDF (capped at 10 a month and not needed). Read every hit whose title or snippet could concern the letter, its sender or recipient in that decade; skip only hits that are plainly about something else and add to that target's AUDIT.md a section `## JSTOR (owner's machine, <date>)` with the citation and one sentence on what it says. Change no N-class.

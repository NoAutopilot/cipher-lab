LANE R7 ATF55V -- fr2933-salviati-1525: glyph-atlas re-pass on leaf f55v to merge near-duplicate types (Sonnet, cap $6, box 60 minutes).
...
the effective noise. f55v is one of the two lowest-agreement leaves (f55v 77.5%, f57v settled by a three-way pass C).
Job:
(1) List every code+mark type that occurs on f55v (ciphertext_f55v.tsv; `python3 control/codemark_curve.py stats --leaves all` for pooled counts).
(2) For each type, cut crops of its occurrences on f55v with `tools/glyph_atlas.py crop --out glyphs --sid ...` (rebuild glyphs/crops first with
...
(3) Decide merges by eye on the crops, never by frequency: a merge is two type labels you cannot tell apart on the image across at least 3
    occurrences each, or one label that is a pass's spelling of the other. Write ciphers/fr2933-salviati-1525/merges_f55v.tsv:
    from_type, to_type, n_occurrences_f55v, evidence (crop file names), confidence (sure/likely). A merge applies codebook-wide.
...
(4) Re-score: pooled type count and per-leaf counts with the merges applied.
    You own the code change: add `--merge FILE [FILE ...]` to control/codemark_curve.py (stats, control, target all read it; it maps from_type->to_type on every leaf's stream before counting or building), with --help text and a case in its offline test if one exists (else a tools/tests/ test). Push that change as soon as it works so AT57V can use it.
...
Write a section "## ATF55V: atlas re-pass on f55v (25 Sept 2026, LANE R7)" to ciphers/fr2933-salviati-1525/leafnotes/atlas_f55v.md (not NOTES.md; two
workers share the folder), with a table: types on f55v before/after, pooled before/after, merges by confidence, keep_distinct count.
ROOM done: "done: for LANE R7: salviati atlas f55v merges <n sure>/<n likely>, f55v types <a> -> <b>, pooled 223 -> <c> (gate <=178)".

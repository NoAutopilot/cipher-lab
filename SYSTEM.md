...
| `tools/lq_answer_check.py` | Bounces a desk-runner "no items" that lacks the holding-catalogue record and quoted availability flag (`tools/data/catalogue_ladders.tsv`) | CLAUDE.md Access playbook, Usage 8a | PR-LAND worker before landing an `[LQ-]` row | a parent |
...
| Key work: `key_crossmatch.py`, `interlinear_align.py`, `thurloe_extract.py` | Try every key on every ciphertext (KEY-CROSSMATCH.tsv, KEY-OFFICES.tsv); known-plaintext alignment; printed-cipher extraction | docstrings; CLAUDE.md Usage 8 | lane workers | tool builders |
...
| Other root TSVs: `KEY-CROSSMATCH.tsv`, `KEY-OFFICES.tsv`, `OPEN-INDEX-RESULTS.tsv`, `POOLS.tsv`, `QUEUE-github-held.tsv` | Key-by-ciphertext sweep results; key office register; open-index scholarship results; sign pools (Pipeline 3 selection rule); solver-repo-held queue rows | their tools and headers | key_crossmatch.py; verifiers; scouts | the lane that owns them |

...
| Key work: `key_crossmatch.py`, `key_design.py`, `design_prior.py`, `interlinear_align.py`, `thurloe_extract.py` | Try every key on every ciphertext (KEY-CROSSMATCH.tsv, KEY-OFFICES.tsv; gate fitted by `--calibrate` on the verified readings, KEY-CROSSMATCH-CAL.tsv + tools/data/key_crossmatch_gate.json; nightly `--since-hours 25 --post-room`); structural signature of every key (KEY-DESIGN.tsv, `--check`) and the design-class prior for an unread ciphertext with a shuffled control (KEY-DESIGN-CAL.tsv calibration, KEY-DESIGN-PRIORS.tsv sweep of unkeyed open/partial ciphertexts); known-plaintext alignment; printed-cipher extraction | docstrings; CLAUDE.md Usage 8 | lane workers | tool builders |
